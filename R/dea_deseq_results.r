...
# Configuration
export_config <- list(
    default_de_params = list(min_padj = 0.05, min_log2fc = 0),
    default_table_params = list(direction = "both", n_genes = 25),
    default_volcano_params = list(n_labels = 15),
    default_heatmap_params = list(
...
#' @param de_params DE parameters
#' @param table_params Table parameters
#' @param volcano_params Volcano plot parameters
#' @param heatmap_params Heatmap parameters
#' @param analysis_type Type of analysis
validate_export_params <- function(experiment_name, de_params, table_params, 
                                 volcano_params, heatmap_params, analysis_type) {
    if (is.null(experiment_name) || nchar(experiment_name) == 0) {
        stop("experiment_name cannot be empty")
    }
    
    if (!analysis_type %in% export_config$valid_analysis_types) {
        stop("Invalid analysis_type. Must be one of: ", paste(export_config$valid_analysis_types, collapse = ", "))
    }
    
    if (!table_params$direction %in% export_config$valid_directions) {
        stop("Invalid direction. Must be one of: ", paste(export_config$valid_directions, collapse = ", "))
    }
    
    if (!heatmap_params$direction %in% export_config$valid_directions) {
        stop("Invalid heatmap direction. Must be one of: ", paste(export_config$valid_directions, collapse = ", "))
    }
}
...
#' Extract design formula information
#' @param dds DESeq2 dataset
#' @return List with design information
extract_design_info <- function(dds) {
    design <- as.character(dds@design)[2]
...
#' Process simple analysis results
#' @param dea_results DESeq2 results
#' @param experiment_obj Experiment object
#' @param de_params DE parameters
#' @param table_params Table parameters
#' @param volcano_params Volcano parameters
#' @param heatmap_params Heatmap parameters
#' @param analysis_type Analysis type
process_simple_analysis <- function(dea_results, experiment_obj, de_params, 
                                  table_params, volcano_params, heatmap_params, 
                                  analysis_type) {
    for (contrast_name in names(dea_results)) {
        message("\nProcessing contrast: ", contrast_name)
        
        # Extract design formula information
        design_info <- extract_design_info(dea_results[[contrast_name]]$dds)
        
        # Add comparison info to the result
        dea_results[[contrast_name]]$comparison_info <- list(
            variable = design_info$main_variable,
...
#' Process complex analysis results
#' @param dea_results DESeq2 results
#' @param experiment_obj Experiment object
#' @param de_params DE parameters
#' @param table_params Table parameters
...
#' @param heatmap_params Heatmap parameters
#' @param analysis_type Analysis type
process_complex_analysis <- function(dea_results, experiment_obj, de_params, 
                                   table_params, volcano_params, heatmap_params, 
                                   analysis_type) {
    message("Processing complex analysis results...")
    
    # Extract design formula from dds object
    design_info <- extract_design_info(dea_results$dds)
    ...
    for (contrast_name in names(dea_results$deseq_results)) {
        ...
        result <- list(
            ...
            comparison_info = list(
                ...
                experimental = strsplit(contrast_name, "_vs_")[[1]][1],
                control = strsplit(contrast_name, "_vs_")[[1]][2],
                contrasts = dea_results$comparison_info$contrasts
            )
        )
...
#' Process individual contrast with error handling
...
process_contrast <- function(result, experiment_obj, contrast_name,
    ...
    tryCatch({
        ...
        message("Error in CSV export: ", e$message)
    })
    gc()
    ...
    tryCatch({
        ...
        message("Error in table generation: ", e$message)
    })
    gc()
    ...
    tryCatch({
        ...
        message("Error in volcano plot generation: ", e$message)
    })
    gc()
    ...
    tryCatch({
        ...
        message("Error in heatmap generation: ", e$message)
    })
    gc()
}
...
export_de_results <- function(result, experiment_obj, direction = "both",
    ...
    # Create DE results directory
    de_dir <- here("results", experiment_obj$metadata$config$name, "de", "deseq2", analysis_type)
...
#' Create DE table for simple analysis
#' @param result Result object
#' @param direction Direction of change
...
#' Create DE table for complex analysis
#' @param result Result object
#' @param direction Direction of change
#' @param n_genes Number of genes to include
#' @param min_padj Minimum adjusted p-value
...
#' @param contrast_name Contrast name
#' @param analysis_type Analysis type
#' @return Path to saved table file
generate_de_tables <- function(result, experiment_obj, direction = "both",
    ...
    tables_dir <- here("results", experiment_obj$metadata$config$name, "de", "deseq2",
                      analysis_type, "tables")
    dir.create(tables_dir, recursive = TRUE, showWarnings = FALSE)
    
    # Use appropriate function based on analysis type
    table_data <- if (analysis_type == "complex") {
        create_complex_de_table(result, direction, n_genes, min_padj, min_log2fc, contrast_name)
    } else {
...
#' Create volcano plot
#' @param plot_data Volcano plot data
#' @param title Plot title
#' @param n_labels Number of labels
#' @param analysis_type Analysis type
#' @return List with plot and dimensions
...
generate_volcano_plot <- function(result, experiment_obj, n_labels = 15,
    ...
    plots_dir <- here("results", experiment_obj$metadata$config$name, "de", "deseq2",
...
#' @param heatmap_data Heatmap data
#' @param result Result object
#' @param title Plot title
#' @param n_clusters Number of clusters
#' @param analysis_type Analysis type
#' @return List with heatmap and dimensions
...
generate_complex_heatmap <- function(result, experiment_obj, direction = "both",
    ...
    base_dir <- here("results", experiment_obj$metadata$config$name)
...
#' Main export function with comprehensive workflow
#' @param experiment_name Name of experiment
#' @param de_params DE parameters
#' @param table_params Table parameters
#' @param volcano_params Volcano plot parameters
...
export_all_result_types <- function(experiment_name,
                                  de_params = export_config$default_de_params,
                                  table_params = export_config$default_table_params,
                                  volcano_params = export_config$default_volcano_params,
                                  heatmap_params = export_config$default_heatmap_params,
                                  analysis_type = "simple") {
    message("Starting export of all result types...")
    
    # Validate parameters
    validate_export_params(experiment_name, de_params, table_params, 
                         volcano_params, heatmap_params, analysis_type)
    ...
    # Create output directories
    create_output_directories(experiment_name, analysis_type)
    
    # Process results based on analysis type
    if (analysis_type == "complex") {
        process_complex_analysis(dea_results, experiment_obj, de_params, 
                               table_params, volcano_params, heatmap_params, 
                               analysis_type)
    } else {
        process_simple_analysis(dea_results, experiment_obj, de_params, 
                              table_params, volcano_params, heatmap_params, 
                              analysis_type)
    }
...
message("DESeq2 results export functions loaded successfully. Use export_all_result_types() to export results in multiple formats.")
